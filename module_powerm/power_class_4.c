...
static void activate(void)
{
    ...
    set_max_gpu_power_level(MAX_GPU_LV_P4);
    set_gov_powersave_bias(PS_BIAS_VLOW);
}

static void update(void){
    printk(KERN_INFO POWER_TAG": Update power class 4\n");
    set_backlight_level(BRIGHTNESS_P4);
}

const struct power_class_ops *POWER_OPS_4 = &((struct power_class_ops){ activate,update });
